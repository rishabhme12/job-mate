...
- **Full regression:** `npm run regression` (runs unit then prompts for E2E). E2E steps are in [docs/REGRESSION_RUN.md](REGRESSION_RUN.md)—run them via Chrome DevTools MCP or manually (requires **logged-in LinkedIn** + Jobs Hero in Chrome); then set Status to pass/fail for E-1, E-2, E-3.
